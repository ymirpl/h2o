...
public class NeuralNet extends ValidatedJob {
  ...
  void startTrain() {
    ...
    if (mode != ExecutionMode.Serial) {
      ...
      Trainer pretrainer = new Direct(ls, (double)serial_rows/num_rows, self());
      pretrainer.start();
      pretrainer.join();
    }
    ...
    Thread monitor = new Thread() {
      ...
      @Override public void run() {
        try {
          ...
          do {
            final long interval = (long)(2 * 1000); //time between evaluations
