...
(function() {
  ...
  module.controller('PreviewController', function($scope, $state, $log, $timeout, $rootScope, ParsePreviewService, ParseGetHeaderService, ParseService, ParseSettingsService, ErrorAndStatusService, DialogService) {
    ...
    $scope.filesAndHeaders = ParseSettingsService.filesAndHeaderFiles();
    $scope.$watchCollection('ParseSettingsService.additionalHeaderFiles', function(newValue, oldValue) {
      return $scope.filesAndHeaders = ParseSettingsService.filesAndHeaderFiles();
    });
    $scope.$watch('ParseSettingsService.headerFile', function(newValue, oldValue) {
      var _ref;
      if (!(_ref = ParseSettingsService.headerFile, __indexOf.call(ParseSettingsService.files, _ref) >= 0)) {
        return ParseGetHeaderService.changeHeaderTo(ParseSettingsService.headerFile);
      }
    });
...
(function() {
  ...
  module.service('ParseSettingsService', function($log, $rootScope) {
    ...
    this.filesAndHeaderFiles = function() {
      var file, l;
      l = (function() {
        var _i, _len, _ref, _results;
        _ref = this.files;
      ...
      l = l.concat(this.additionalHeaderFiles);
      return l;
    };
    ...
    this.headerFromOutsideBasicFileList = function() {
      var f, _ref;
      if ((_this.headerFile != null) && _this.skipHeader === false) {
        if (!(_ref = _this.headerFile, __indexOf.call((function() {
          var _i, _len, _ref1, _results;
          _ref1 = this.files;
          _results = [];
          for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
    ...
    this.updateFieldsFromParserConfigData = function() {
      ...
      if (_this.parserConfigData.header_file != null) {
        if (_this.typeIsArray(_this.parserConfigData.header_file)) {
          _this.headerFile = _this.parserConfigData.header_file[0];
        } else {
    ...
    this.setColumnsFromHeader = function(header_data) {
      var h, i, _i, _len;
      for (i = _i = 0, _len = header_data.length; _i < _len; i = ++_i) {
        h = header_data[i];
        _this.columns[i].header = h;
      }
      return _this.didChangeColumnSettings();
    };
    this.updateParserConfigDataFromFields = function() {
      ...
      for (k in newParserConfigData) {
        ...
        if (k !== "additionalHeaderFiles") {
          if (k === 'headerFile' && (newParserConfigData[k] != null) && _this.typeIsArray(newParserConfigData[k])) {
            newParserConfigData[k] = newParserConfigData[k][0];
          }
...
(function() {
  ...
  module.service('ParseGetHeaderService', function($http, $log, $rootScope, $q, ParseSettingsService, ErrorAndStatusService) {
    ...
    this.didReceiveResponse = function(data, file) {
      ...
      ParseSettingsService.headerSeparator = data.header_separator;
      if (data.header != null) {
        if (data.header.length === ParseSettingsService.numCols) {
          ParseSettingsService.setColumnsFromHeader(data.header);
        } else {
          ErrorAndStatusService.clearErrors({
          ...
          ErrorAndStatusService.setStatus('ParseGetHeaderService', "error");
          ErrorAndStatusService.addError('ParseGetHeaderService', {
            message: "Selected header has wrong number of columns."
          });
        }
      }
      return ParseSettingsService.saveWizardState();
    };
